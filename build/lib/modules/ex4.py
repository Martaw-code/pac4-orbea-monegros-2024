...
def add_clean_club_column(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def groupby_club_clean(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    :param df: DataFrame amb la columna 'club_clean'.
    :return: DataFrame amb ['club_clean', 'count'] ordenat desc.
    """
    grouped = df.groupby('club_clean').size().reset_index(name='count')
    grouped = grouped.sort_values('count', ascending=False)
    return grouped

def run_ex4(df: pd.DataFrame) -> pd.DataFrame:
    """
    Executa l'Exercici 4, rebent el df resultant de l'ex3.
    """
    print("\n--- EXERCICI 4 ---")
    df = add_clean_club_column(df)
    print("1) 15 primers valors amb columna 'club_clean':")
    print(df.head(15)[['club', 'club_clean']])

    club_grouped = groupby_club_clean(df)
